...
class DeepZoomImageTiler(object):
    ...
    def _write_tiles(self):
        ...
        #print(f'self._dz.level_count :- {self._dz.level_count}, self._target_levels :- {self._target_levels}, target_levels :- {target_levels}')
        mag_list = [int(self._mag_base/2**i) for i in self._target_levels]
        #print(f'mag_list :- {mag_list}')
        #print(f'self._dz.level_tiles :- {self._dz.level_tiles}')
        mag_idx = 0
...
def nested_patches(img_slide, out_base, level=(0,), ext='jpeg'):
    ...
    n_levels = len(glob.glob('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files/*'))
    bag_path = os.path.join(out_base, img_class, img_name)
    os.makedirs(bag_path, exist_ok=True)
    if len(level)==1:
        patches = glob.glob(os.path.join('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files', '*', '*.'+ext))
        ...
        levels = [int(os.path.basename(i)) for i in glob.glob(os.path.join('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files', '*'))]
        levels.sort()
        low_patches = glob.glob(os.path.join('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files', str(levels[0]), '*.'+ext))
        for i, low_patch in enumerate(low_patches):
            ...
            for x_pos in high_x_list:
                for y_pos in high_y_list:
                    high_patch = glob.glob(os.path.join('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files', str(levels[1]), '{}_{}.'.format(x_pos, y_pos)+ext))
                    if len(high_patch)!=0:
...
if __name__ == '__main__':
    ...
    path_base = os.path.join('/scratch/shubham.ojha/WSI', args.dataset)
    if len(levels) == 2:
        out_base = os.path.join('/scratch/shubham.ojha/WSI', args.dataset, 'pyramid')
    else:
        out_base = os.path.join('/scratch/shubham.ojha/WSI', args.dataset, 'single')
    all_slides = glob.glob(os.path.join(path_base, '*/*.'+args.slide_format)) +  glob.glob(os.path.join(path_base, '*/*/*.'+args.slide_format))
    print(all_slides[0])
    
    
    for idx, c_slide in enumerate(all_slides[:1]):
        print('Process slide {}/{}'.format(idx+1, len(all_slides)))
        DeepZoomStaticTiler(c_slide, '/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp', levels, args.base_mag, args.objective, args.format, args.tile_size, args.overlap, True, args.quality, args.workers, args.background_t).run()
        nested_patches(c_slide, out_base, levels, ext=args.format)
        shutil.rmtree('/scratch/shubham.ojha/WSI/tcga_brain/WSI_temp_files') 
