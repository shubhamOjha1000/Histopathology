...
def compute_tree_feats(args, bags_list, embedder_low, embedder_high, save_path=None):
    ...
    with torch.no_grad():
        for i in range(0, num_bags): 
            ...
            for iteration, batch in enumerate(dataloader):
                patches = batch['input'].float().cuda()
                logging.info(f'patches :- {patches.shape}')
...
def main():
    ...
    parser.add_argument('-p', '--path_to_WSI', type=str, default='/scratch/shubham.ojha/WSI', help = 'Path to WSI folder')
    parser.add_argument('--pyramid_no', type=str, default='pyramid', help = 'Pyramid No')
    parser.add_argument('--feats_dir_name', type=str, default='patch_features_resnet50', help = 'Feature Directory Name')
    ...
    if args.backbone == 'resnet18':
        resnet = models.resnet18(pretrained=True, norm_layer=norm)
        num_feats = 512
    if args.backbone == 'resnet34':
        resnet = models.resnet34(pretrained=True, norm_layer=norm)
        num_feats = 512
    if args.backbone == 'resnet50':
        resnet = models.resnet50(pretrained=True, norm_layer=norm)
        num_feats = 2048
    if args.backbone == 'resnet101':
        resnet = models.resnet101(pretrained=True, norm_layer=norm)
        num_feats = 2048
    ...
    if args.magnification == 'tree' and args.weights_high != None and args.weights_low != None:
        ...
        if args.weights_high == 'ImageNet' or args.weights_low == 'ImageNet' or args.weights== 'ImageNet':
            ...
            weight_path = os.path.join('/home/shubham.ojha/dsmil-wsi', 'model-v1.pth')
            ...
            torch.save(new_state_dict, os.path.join('embedder', args.dataset, 'embedder-high-resnet50.pth'))

            #weight_path = os.path.join('simclr', 'runs', args.weights_low, 'checkpoints', 'model.pth')
            weight_path = os.path.join('/home/shubham.ojha/dsmil-wsi', 'model-low-v1.pth')
            state_dict_weights = torch.load(weight_path)
            for i in range(4):
                state_dict_weights.popitem()
            ...
            os.makedirs(os.path.join('embedder', args.dataset), exist_ok=True)
            torch.save(new_state_dict, os.path.join('embedder', args.dataset, 'embedder-low-resnet50.pth'))
    ...
    if args.magnification == 'tree' or args.magnification == 'low' or args.magnification == 'high' :
        bags_path = os.path.join(args.path_to_WSI, args.dataset, args.pyramid_no, '*', '*')
        #bags_path = os.path.join('/scratch/shubham.ojha/WSI/tcga_lung/', args.pyramid_no, '*', '*')
    else:
        bags_path = os.path.join('WSI', args.dataset, 'single', '*', '*')
    bags_path = '/scratch/ekansh.chauhan/DATA/IN Brain-0007(a)'
    bags_list = sorted(glob.glob(bags_path))
    #logging.info(f'bags_path :- {bags_list}')
    
    
    feats_path = os.path.join(args.path_to_WSI, args.dataset,args.feats_dir_name)
        
    os.makedirs(feats_path, exist_ok=True)
    #bags_list = glob.glob(bags_path)
    #logging.info(f'bags_list :- {len(bags_list)}')
    ...
    for i, item in enumerate(n_classes):
        ...
        #logging.info(f'item.split(os.path.sep)[2] :- {item.split(os.path.sep)[-2]}')
        bag_csvs = glob.glob(os.path.join(item, '*.csv'))
        #logging.info(f'bag_csvs :- {bag_csvs}')
    ...
    bags_path.to_csv(os.path.join(feats_path, args.dataset+'.csv'), index=False)
    """
